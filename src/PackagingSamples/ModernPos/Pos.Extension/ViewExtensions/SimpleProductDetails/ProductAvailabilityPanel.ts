...
export default class ProductAvailabilityPanel extends SimpleProductDetailsCustomControlBase {
  ...
  private static readonly TEMPLATE_ID: string =
    "Golrang_Pos_Extensibility_Samples_ProductAvailabilityPanel";
  ...
  public onReady(element: HTMLElement): void {
    let templateElement: HTMLElement = document.getElementById(
      ProductAvailabilityPanel.TEMPLATE_ID
    );
    let templateClone: Node = templateElement.cloneNode(true);
    element.appendChild(templateClone);
    let titleElement: HTMLElement = element.querySelector(
      "#Golrang_Pos_Extensibility_Samples_ProductAvailabilityPanel_TitleElement"
    );
    titleElement.innerText = this.title;

    let dataListOptions: Readonly<
      Controls.IDataListOptions<ProxyEntities.OrgUnitAvailability>
    > = {
      columns: [
        {
          title: "Location",
        ...
        },
        {
          ...
          computeValue: (value: ProxyEntities.OrgUnitAvailability): string => {
            return ArrayExtensions.hasElements(value.ItemAvailabilities)
              ? value.ItemAvailabilities[0].AvailableQuantity.toString()
              : "0";
          },
        },
        {
          ...
          computeValue: (value: ProxyEntities.OrgUnitAvailability): string => {
            return ArrayExtensions.hasElements(value.ItemAvailabilities)
              ? value.ItemAvailabilities[0].PhysicalReserved.toString()
              : "0";
          },
        },
        {
          ...
          computeValue: (value: ProxyEntities.OrgUnitAvailability): string => {
            return ArrayExtensions.hasElements(value.ItemAvailabilities)
              ? value.ItemAvailabilities[0].OrderedSum.toString()
              : "0";
          },
        },
      ],
    ...
    let dataListRootElem: HTMLDivElement = element.querySelector(
      "#Golrang_Pos_Extensibility_Samples_ProductAvailabilityPanel_DataList"
    ) as HTMLDivElement;
  ...
  public init(state: ISimpleProductDetailsCustomControlState): void {
    ...
    if (!this._state.isSelectionMode) {
      ...
      this.context.runtime
        ...
        .then(
          (
            result: ClientEntities.ICancelableDataResult<InventoryLookupOperationResponse>
          ) => {
            if (!result.canceled) {
