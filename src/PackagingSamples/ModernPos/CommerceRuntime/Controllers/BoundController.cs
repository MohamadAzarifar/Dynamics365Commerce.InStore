...
namespace Golrang.CommerceRuntime.Hosting
{
    ...
    /// <summary>
    /// An extension controller to handle requests to the example entity set.
    /// </summary>
    [RoutePrefix("BoundController")]
    [BindEntity(typeof(DataModel.ExampleEntity))]
    public class BoundController : IController
    {
        ...
        [HttpPost]
        [Authorization(CommerceRoles.Customer, CommerceRoles.Device, CommerceRoles.Employee)]
        public async Task<bool> UpdateExampleEntity(IEndpointContext context, [EntityKey] long key, DataModel.ExampleEntity updatedEntity)
        {
