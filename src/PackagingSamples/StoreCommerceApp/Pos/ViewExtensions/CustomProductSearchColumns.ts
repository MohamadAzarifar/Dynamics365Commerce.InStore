...
export default (context: ICustomColumnsContext): IProductSearchColumn[] => {
  return [
    ...
    },
    {
      title: "Price",
      computeValue: (row: ProxyEntities.ProductSearchResult): string => {
        return CurrencyFormatter.toCurrency(row.Price);
      },
      ratio: 20,
      collapseOrder: 1,
      minWidth: 100,
    ...
    },
    {
      ...
      computeValue: (row: ProxyEntities.ProductSearchResult): string => {
        ...
        let commerceProperty = ArrayExtensions.firstOrUndefined(
          row.ExtensionProperties,
          (prop: ProxyEntities.CommerceProperty): boolean => {
            return prop.Key === "Golrang_PRODUCT_VERSION";
          }
        );
        if (
          ObjectExtensions.isNullOrUndefined(commerceProperty) ||
          ObjectExtensions.isNullOrUndefined(commerceProperty.Value) ||
          StringExtensions.isNullOrWhitespace(
            commerceProperty.Value.StringValue
          )
        ) {
