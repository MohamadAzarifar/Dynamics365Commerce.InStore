...
import {
  ...
  CustomViewControllerExecuteCommandArgs,
  ICommand,
  Icons,
...
import {
  ...
  ShowMessageDialogClientResponse,
...
import {
  ...
  GetScanResultClientResponse,
} from "PosApi/Consume/ScanResults";
...
export default class GasPumpStatusView extends CustomViewControllerBase {
  ...
  private static readonly DATA_LIST_QUERY_SELECTOR: string =
    "#Golrang_gasPumpStatusView_DataList";
  ...
  private static readonly START_STOP_COMMAND_NAME: string =
    "StartStopPumpCommand";
  ...
  constructor(context: ICustomViewControllerContext) {
        ...
        commands: [
          ...
          },
          {
            ...
            execute: (args: CustomViewControllerExecuteCommandArgs): void => {
              ...
              GasStationDataStore.instance
                .stopAllPumpsAsync(this.context)
                .then((updatedPumps): void => {
          ...
          },
          {
            ...
            execute: (args: CustomViewControllerExecuteCommandArgs): void => {
              this.state.isProcessing = true;
              GasStationDataStore.instance
                .startAllPumpsAsync(this.context)
                .then((updatedPumps): void => {
                  this.state.isProcessing = false;
                })
                .catch((reason: any) => {
    ...
    this.selectedPumpDescription = ko.computed((): string => {
      return ObjectExtensions.isNullOrUndefined(this.selectedGasPump())
        ? ""
        : this.selectedGasPump().Description;
    }, this);
    this.selectedPumpStatus = ko.computed((): string => {
      ...
      let status: GasPumpStatus =
        this.selectedGasPump().State.GasPumpStatusValue;
      switch (status) {
    ...
    this.isSelectedGasPumpPumping = ko.computed((): boolean => {
      ...
      return (
        this.selectedGasPump().State.GasPumpStatusValue ===
          GasPumpStatus.Pumping ||
        this.selectedGasPump().State.GasPumpStatusValue ===
          GasPumpStatus.PumpingComplete
      );
    }, this);
  ...
  private get _stopAllCommand(): ICommand {
    return ArrayExtensions.firstOrUndefined(
      ...
      (c) => c.name === GasPumpStatusView.STOP_ALL_COMMAND_NAME
    );
  }
  ...
  private get _startAllCommand(): ICommand {
    return ArrayExtensions.firstOrUndefined(
      this.state.commandBar.commands,
      (c) => c.name === GasPumpStatusView.START_ALL_COMMAND_NAME
    );
  }
  ...
  private get _checkoutCommand(): ICommand {
    return ArrayExtensions.firstOrUndefined(
      this.state.commandBar.commands,
      (c) => c.name === GasPumpStatusView.CHECKOUT_COMMAND_NAME
    );
  }
  ...
  private get _toggleStartStopCommand(): ICommand {
    return ArrayExtensions.firstOrUndefined(
      this.state.commandBar.commands,
      (c) => c.name === GasPumpStatusView.START_STOP_COMMAND_NAME
    );
  }
  ...
  public onReady(element: HTMLElement): void {
    let dataListElement: HTMLDivElement = element.querySelector(
      GasPumpStatusView.DATA_LIST_QUERY_SELECTOR
    ) as HTMLDivElement;
      ...
      "DataList",
      {
        columns: [
          ...
          },
          {
            collapseOrder: 1,
            computeValue: (row: Entities.GasPump): string => {
              return ObjectExtensions.isNullOrUndefined(
                row.State.LastUpdateTime
              )
                ? "Unknown"
        ...
        equalityComparer: (
          left: Entities.GasPump,
          right: Entities.GasPump
        ): boolean => {
    ...
    this._dataList.addEventListener(
      "SelectionChanged",
      (selectionData: { items: Entities.GasPump[] }): void => {
        if (ArrayExtensions.hasElements(selectionData.items)) {
          this.selectedGasPump(selectionData.items[0]);
          this._checkoutCommand.canExecute =
            selectionData.items[0].State.GasPumpStatusValue ===
            GasPumpStatus.PumpingComplete;
   ...
   */
  public onShown(): void {
    this._gasPumpChangedHandlerId =
      GasStationDataStore.instance.addPumpStatusChangedHandler(() => {
        this._refreshPumps(GasStationDataStore.instance.pumps);
      });
  }
  ...
  private _refreshPumps(pumps: ReadonlyArray<Entities.GasPump>): void {
    ...
    let pumpInEmergency: Entities.GasPump = ArrayExtensions.firstOrUndefined(
      pumps,
      (p) => p.State.GasPumpStatusValue === GasPumpStatus.Emergency
    );
    ...
    this._stopAllCommand.canExecute = pumps.some(
      (p) =>
        p.State.GasPumpStatusValue !== GasPumpStatus.Stopped &&
        p.State.GasPumpStatusValue !== GasPumpStatus.Unknown
    );

    // Allow start all if some pumps are not pumping.
    this._startAllCommand.canExecute = pumps.some(
      (p) =>
        p.State.GasPumpStatusValue === GasPumpStatus.Stopped ||
        p.State.GasPumpStatusValue === GasPumpStatus.Unknown
    );
  ...
  private _addGasolineToCartAsync(): Promise<void> {
    ...
    return this.context.runtime
      .executeAsync(extensionPropertyRequest)
      .then(
        (
          result
        ): Promise<ICancelableDataResult<GetScanResultClientResponse>> => {
          if (result.canceled) {
            return Promise.resolve({ canceled: true, data: null });
          }

          let getScanResult: GetScanResultClientRequest<GetScanResultClientResponse> =
            new GetScanResultClientRequest(
              GasStationDataStore.instance.gasStationDetails.GasolineItemId
            );
      ...
      .then(
        (
          result
        ): Promise<ICancelableDataResult<AddItemToCartOperationResponse>> => {
          if (result.canceled) {
            return Promise.resolve({ canceled: true, data: null });
          }
      ...
      .then((result) => {
        ...
        this.context.navigator.navigateToPOSView(
          "CartView",
          new ClientEntities.CartViewNavigationParameters(correlationId)
        );
      })
  ...
  private _enableDisablePumpAsync(): Promise<void> {
    ...
    if (
      oldState.GasPumpStatusValue === GasPumpStatus.Stopped ||
      oldState.GasPumpStatusValue === GasPumpStatus.Unknown
    ) {
    ...
    return GasStationDataStore.instance
      .updatePumpStatusAsync(this.context, this.selectedGasPump().Id, newState)
      .then((updatedPump: Entities.GasPump): void => {
        this.state.isProcessing = false; // Setting isProcessing value to false will hide the processing indicator/spinner.
      })
      .catch((reason: any): void => {
        this.state.isProcessing = false; // Setting isProcessing value to false will hide the processing indicator/spinner.
   ...
   */
  private static _displayErrorAsync(
