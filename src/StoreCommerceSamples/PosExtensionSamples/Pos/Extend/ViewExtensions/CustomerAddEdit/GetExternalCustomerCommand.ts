...
export default class GetExternalCustomerCommand extends CustomerAddEditView.CustomerAddEditExtensionCommandBase {
   ...
   */
  constructor(
    context: IExtensionCommandContext<CustomerAddEditView.ICustomerAddEditToExtensionCommandMessageTypeMap>
  ) {
    super(context);
   ...
   */
  protected init(
    state: CustomerAddEditView.ICustomerAddEditExtensionCommandState
  ): void {
  ...
  protected execute(): void {
    ...
    let dialogRequest: ShowAlphanumericInputDialogClientRequest<ShowAlphanumericInputDialogClientResponse> =
      new ShowAlphanumericInputDialogClientRequest<ShowAlphanumericInputDialogClientResponse>(
        inputAlphanumericDialogOptions
      );

    this.context.runtime
      .executeAsync(dialogRequest)
      .then(
        (
          dialogResponse: ClientEntities.ICancelableDataResult<ShowAlphanumericInputDialogClientResponse>
        ) => {
          if (!dialogResponse.canceled) {
            let customer: ProxyEntities.Customer = this.customer;
            let primaryAddress: ProxyEntities.Address =
              new ProxyEntities.AddressClass();

            if (dialogResponse.data.result.value === "1") {
              primaryAddress.AddressTypeValue =
                ClientEntities.ExtensibleAddressType.Business.Value;
              primaryAddress.City = "Redmond";
              primaryAddress.IsPrimary = true;
              primaryAddress.Name = "Golrang Consulting USA";
              ...
              customer.Name = "Golrang Consulting USA";
              customer.Addresses = addresses;
            } else {
              primaryAddress.AddressTypeValue =
                ClientEntities.ExtensibleAddressType.Home.Value;
              primaryAddress.City = "Redmond";
              primaryAddress.IsPrimary = true;
