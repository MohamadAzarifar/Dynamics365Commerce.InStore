...
export default class GetExternalAddressCommand extends AddressAddEditView.AddressAddEditExtensionCommandBase {
   ...
   */
  constructor(
    context: IExtensionCommandContext<AddressAddEditView.IAddressAddEditToExtensionCommandMessageTypeMap>
  ) {
    super(context);
   ...
   */
  protected init(
    state: AddressAddEditView.IAddressAddEditExtensionCommandState
  ): void {
      ...
      .then(
        (
          ...
          if (!dialogResponse.canceled) {
            let address: ProxyEntities.Address =
            ...
            if (dialogResponse.data.result.value === "1") {
              ...
              address.Name = "Golrang Consulting USA";
              address.State = "WA";
